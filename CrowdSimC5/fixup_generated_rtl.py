    ...
    for kernel in kernels:
        ...
        for line in lines: 
            ...
            if in_port_list == 1 and re.match('\s*\);\s*\n', line, re.S) != None:
                # FIXME: This works only for one kernel
                in_port_list = 0  
...
shutil.copyfile('../common/rtl_src/axi_cache_secruity_bridge/AXI_cache_secruity_bridge_hw.tcl', os.path.join(proj, 'ip','axi_cache_secruity_bridge','AXI_cache_secruity_bridge_hw.tcl'))
shutil.copyfile('../common/rtl_src/axi_cache_secruity_bridge/axi_cache_secruity_bridge.v', os.path.join(proj, 'ip','axi_cache_secruity_bridge','axi_cache_secruity_bridge.v'))
...
shutil.copyfile('../common/rtl_src/lock_server/lock_server_hw.tcl', os.path.join(proj, 'ip','lock_server','lock_server_hw.tcl'))
shutil.copyfile('../common/rtl_src/lock_server/lock_server.vhd', os.path.join(proj, 'ip','lock_server','lock_server.vhd'))

# run tcl scripts to modify Qsys system
shutil.copyfile('iface.tcl', os.path.join(proj, 'iface.tcl'))
shutil.copyfile('svm_system.tcl', os.path.join(proj, 'svm_system.tcl'))
os.chdir(proj)
